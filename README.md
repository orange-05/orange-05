<div align="center">

# Karthikeyan K

**ML Infrastructure & Applied AI** · Data Analytics → MSc Data Science

[![Remote Internships](https://img.shields.io/badge/Open_to-Remote_Internships-2ea44f?style=flat-square)](mailto:karthikeyankkarthik7@gmail.com)
[![Contract Projects](https://img.shields.io/badge/Open_to-Contract_Projects-2ea44f?style=flat-square)](mailto:karthikeyankkarthik7@gmail.com)
[![Part-time](https://img.shields.io/badge/Open_to-Part--time_Roles-2ea44f?style=flat-square)](mailto:karthikeyankkarthik7@gmail.com)
![Availability](https://img.shields.io/badge/Availability-1--2%20days-blue?style=flat-square)

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/karthikeyan-k-87050a290)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:karthikeyankkarthik7@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/orange-05)

</div>

<br>

<div align="center">

### Stack

<img src="https://skillicons.dev/icons?i=python,sql,docker,terraform,aws,azure,gcp,postgres,mongodb,git,react,ts,prometheus,grafana&theme=dark" />

</div>

<br>

---

### Projects

<table>
<tr>
<td width="100%">

**🔒 DevGuard — Zero-Trust DevSecOps Pipeline**
End-to-end CI/CD: vulnerability scanning, SBOM generation, image signing. Infra provisioned via Terraform, containerized, monitored in real time.
`Terraform` `Docker` `Prometheus` `Grafana` `PostgreSQL`

</td>
</tr>
<tr>
<td width="100%">

**📄 Hybrid LLM — IPO Prospectus Analyzer**
Dual-LLM research system that surfaces risk factors from IPO prospectus documents automatically.
`Python` `LLMs` `Document Analysis`

</td>
</tr>
<tr>
<td width="100%">

**🤖 AIOS — Multi-Agent Organization** <sub>*(in progress)*</sub>
Coordinated AI agents structured like an org — each agent builds a product component (web apps, APIs, SaaS) via task delegation.
`Python` `Agent Orchestration` `LLMs`

</td>
</tr>
<tr>
<td width="100%">

**📊 COVID-19 Global Trends — EDA**
500K+ records analyzed for infection/spread patterns; regression & classification models for case prediction.
`Python` `Pandas` `Scikit-learn`

</td>
</tr>
</table>

<div align="center"><sub><a href="https://github.com/orange-05">→ full project list</a></sub></div>

<br>

---

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=orange-05&show_icons=true&theme=dark&hide_border=true&count_private=true" height="165" />
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=orange-05&layout=compact&theme=dark&hide_border=true" height="165" />

</div>

<br>

---

<div align="center">

**Currently learning:** AWS SAA-C03 · Kubernetes (CKA) · LangChain / Agent Orchestration · System Design

<br>

📬 **Reach out for remote internships, contract work, or part-time collaboration** — [karthikeyankkarthik7@gmail.com](mailto:karthikeyankkarthik7@gmail.com)

</div>
